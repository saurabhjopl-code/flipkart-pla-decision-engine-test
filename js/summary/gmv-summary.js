...
export function renderGMVSummary() {

    const totalRevenue = sum(STATE.data.gmvDaily, "Net Sales (GMV)");
    const totalUnits = sum(STATE.data.gmvDaily, "Net Units");
    const cancel = sum(STATE.data.gmvDaily, "Cancellation Amount");
    const returns = sum(STATE.data.gmvDaily, "Return Amount");
